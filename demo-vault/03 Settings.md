...
- `attachmentLinksPrefix`
  - the text inserted once, before the first attachment link. Empty by default.
- `attachmentLinksDelimiter`
  - the text inserted between attachment links. A blank line (two newlines) by default, so each attachment lands on its own paragraph.
- `attachmentLinksSuffix`
  - the text inserted once, after the last attachment link. Empty by default.
...
- `shouldShowRibbonIcon`
  - show the paperclip ribbon icon in the left sidebar. Enabled by default. Clicking it inserts attachments into the active note.
- `shouldShowInEditorContextMenu`
  - show the **Insert multiple attachments** item in the editor right-click menu. Enabled by default.
